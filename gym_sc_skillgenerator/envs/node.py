...
class SkillNode(ABC, metaclass=ABCMeta):
    ...
    def __str__(self):
        return f"{self.__class__.__name__}{{ {self.xlable()} }}"
...
class CooldownCheck(SkillNode):
    ...
    def __init__(self, node_id: int, cooldown, *_):
        ...
        # Max cooldown 3 days
        self.cooldown = cooldown * MAX_COOLDOWN + 0.1
    ...
    def get_props(self):
        ...
        props.difficulty += self.cooldown / 5
        return props
...
class DistanceCheck(SkillNode):
    ...
    def __init__(self, node_id: int, min_range: float, max_range: float, *_):
...
class EntityDamageMake(SkillNode):
    ...
    def xlable(self):
        return f"base_damage = {self.base_damage}, accuracy = {self.accuracy}, " \
...
class ExecutorAnimation(SkillNode):
    ...
    def get_props(self):
        ...
        props.fanciness += int(self.animation_idx) + 1
        return props

    def xlable(self):
        return f"animation_idx = {self.animation_idx}"
